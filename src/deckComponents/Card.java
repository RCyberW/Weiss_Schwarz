 ...
 * @author Jia Chen
 * @date May 04, 2012
 * @description Cardv2.java is the test instance of Card.java.
 */
...
public class Card implements Serializable, MouseListener, MouseMotionListener, Comparable<Object>,
    Transferable {
  ...
  public boolean setID(String id) {
    ...
    for (int i = 1; i < newId.length(); i++) {
      if ((Character.isLetter(newId.charAt(i)) && Character.isDigit(newId.charAt(i - 1)))
          || (Character.isSpaceChar(newId.charAt(i)) && Character.isDigit(newId.charAt(i - 1)))) {
        break;
      } else {
        pID += newId.charAt(i);
      }
...
//        img = new ImageIcon(image.getScaledInstance((int) (image.getWidth(null) * 0.44 * 0.79),
...
//
//        img = new ImageIcon(image.getScaledInstance((int) (image.getWidth(null) * 0.44),
//            (int) (image.getHeight(null) * 0.44), Image.SCALE_SMOOTH));
//      }

      imageLabel.setIcon(new ImageIcon(getImageResource()));
  ...
  public JPanel getInfoPane(int w, int h) {
    ...
    JTextField levelLabel = new JTextField("Level: " + (level >= 0 ? level : " -"));
    levelLabel.setEditable(false);
    // levelLabel.setFont(font);

    JTextField costLabel = new JTextField("Cost: " + (cost >= 0 ? cost : " -"));
    ...
    JTextField powerLabel = new JTextField("Power: " + (power > 0 ? power : " -"));
    powerLabel.setEditable(false);
    // powerLabel.setFont(font);

    JTextField damageLabel = new JTextField("Soul: " + (soul > 0 ? soul : " -"));
                ...
                .addGroup(
                    layout
                        ...
                        .addGroup(
                            layout
                                .createSequentialGroup()
                                .addComponent(levelLabel, GroupLayout.PREFERRED_SIZE, 60,
                                    GroupLayout.PREFERRED_SIZE).addComponent(costLabel)))
                .addGroup(
                    layout
                        .createParallelGroup()
                        .addGroup(layout.createSequentialGroup().addComponent(typeLabel))
                        .addGroup(
    ...
    layout.setVerticalGroup(layout
        ...
        .addGroup(
            layout.createParallelGroup().addComponent(idLabel).addComponent(typeLabel)
                .addComponent(soulLabel))
        .addGroup(
            layout.createParallelGroup().addComponent(levelLabel).addComponent(costLabel)
                .addComponent(powerLabel).addComponent(damageLabel))
  ...
  public JPanel displayImage(int w, int h) {
    ...
    try {
      ...
      ImageIcon img;
      if (getImageResource().contains(".gif")) {
        img = new ImageIcon(image.getScaledInstance((int) (image.getWidth(null) * 0.79),
   ...
   * @return
   */
  public boolean meetsRequirement(String sId, String sName, CCode sColor, Type sType, int sLevel,
      int sCost, Trigger sTrigger, int sPower, int sSoul, String sTrait, String sAbility) {
    ...
    if (!sName.isEmpty()) {
      isMet = isMet
          && (cardName.toLowerCase().contains(sName.toLowerCase()) || cardName_e.toLowerCase()
              .contains(sName.toLowerCase()));
    }
    ...
    if (sType != null && sType != deckComponents.Type.ALL) {
      isMet = isMet && (sType == t);
    }
  ...
  public Rectangle getCardBound() {
    ...
    boundBox.setBounds((int) customCanvas.getLocation().x,
        (int) (customCanvas.getLocation().y + gamePlay.Game.translatedY), customCanvas.getWidth(),
        customCanvas.getHeight());
      ...
      customCanvas = new Canvas() {
        ...
        public void paint(Graphics g) {
          ...
          try {
            ...
            at.scale(gamePlay.Game.gameScale, gamePlay.Game.gameScale);

            if (currentState == State.REST || currentState == State.FD_REST) {
              at.translate((after.getHeight(null) - before.getWidth(null)) / 2,
                  (after.getWidth(null) - before.getHeight(null)) / 2);
              if (getT() == Type.CLIMAX && currentState == State.REST) {
                at.rotate(Math.toRadians(-90), before.getWidth(null) / 2,
            ...
            customCanvas.setBounds(getLocation().x, getLocation().y, (int) (after.getWidth(null)),
                (int) (after.getHeight(null)));

            boolean debug = false;

            String outputString = (int) getCardBound().getX() + " + " + getCardBound().width + ","
                + (int) getCardBound().getY() + " + " + getCardBound().height;
            if (debug) {
              g.setColor(Color.BLUE);
              g.fillRect((int) getCardBound().getX(), (int) getCardBound().getY(),
                  getCardBound().width, getCardBound().height);
              g.setColor(Color.RED);
              g.drawRect((int) getCardBound().getX(), (int) getCardBound().getY(),
                  getCardBound().width, getCardBound().height);
  ...
  public static int getMaxInDeck(Card c) {
    if (c.getID().equals("FZ/SE13-24 C") || c.getID().equals("FZ/SE13-26 C")
        || c.getID().equals("MF/S13-034 U") || c.getID().equals("MF/S13-040 C")
        || c.getID().equals("ID/W10-014 C") || c.getID().equals("SG/W19-038 C"))
      return 50;
